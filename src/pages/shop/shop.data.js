const SHOP_DATA = [
  {
    id: 1,
    title: 'Jewelry',
    routeName: 'jewelry',
    items: [
      {
        id: 1,
        name: 'Puffed Heart Floral Locket',
        imageUrl: 'https://i.imgur.com/En9d0an.jpg',
        price: 36
      },
      {
        id: 2,
        name: 'Carnelian Bracelet',
        imageUrl: 'https://i.imgur.com/hcWTVVN.jpg',
        price: 18
      },
      {
        id: 3,
        name: 'London Blue Topaz Pearl Necklace',
        imageUrl: 'https://i.imgur.com/pa6nLHe.jpg',
        price: 78
      },
      {
        id: 4,
        name: 'Blue Geode Gemstone Bracelet',
        imageUrl: 'https://i.imgur.com/SqLEPYp.jpg',
        price: 75
      },
      {
        id: 5,
        name: 'Jasper & White Stone Beaded Necklace',
        imageUrl: 'https://i.imgur.com/Wd8LRJi.jpg',
        price: 24
      },
      {
        id: 6,
        name: 'Teardrop Cocktail Ring',
        imageUrl: 'https://i.imgur.com/OiunzWU.jpg',
        price: 25
      },
      {
        id: 7,
        name: 'Rose Quartz Bangle',
        imageUrl: 'https://i.imgur.com/TuykmXs.jpg',
        price: 54
      },
      {
        id: 8,
        name: 'White Sapphire Drop Necklace ',
        imageUrl: 'https://i.imgur.com/lj3naK4.jpg',
        price: 96
      },
      {
        id: 9,
        name: 'Mixed Blues Stone Bracelet',
        imageUrl: 'https://i.imgur.com/11TJPfd.jpg',
        price: 35
      },
      {
        id: 10,
        name: 'Antique Pocket Watch Necklace',
        imageUrl: 'https://i.imgur.com/0DlwqCS.jpg',
        price: 18
      }
    ]
  },
  {
    id: 2,
    title: 'Candles',
    routeName: 'candles',
    items: [
      {
        id: 1,
        name: 'Glass Tealight Set',
        imageUrl: 'https://i.imgur.com/w4NodZH.jpg',
        price: 20
      },
      {
        id: 2,
        name: 'Zen Garden Candle',
        imageUrl: 'https://i.imgur.com/duxf3IL.jpg',
        price: 28
      },
      {
        id: 3,
        name: 'Bamboo Blossom Candle',
        imageUrl: 'https://i.imgur.com/YoR0pGJ.jpg',
        price: 26
      },
      {
        id: 4,
        name: 'Stone Essence Oil Candle',
        imageUrl: 'https://i.imgur.com/z7KZxJT.jpg',
        price: 55
      },
      {
        id: 5,
        name: 'Carved Rose Candle',
        imageUrl: 'https://i.imgur.com/fqWiF7L.jpg',
        price: 59
      },
      {
        id: 6,
        name: 'Tealight Candle Set',
        imageUrl: 'https://i.imgur.com/l1Jo2mj.jpg',
        price: 11
      },
      {
        id: 7,
        name: 'Beaded Pearl Candle',
        imageUrl: 'https://i.imgur.com/WN7G7NY.jpg',
        price: 36
      },
      {
        id: 8,
        name: 'Cauldron Pot Candle',
        imageUrl: 'https://i.imgur.com/IogxvnS.jpg',
        price: 22
      },
      {
        id: 9,
        name: 'Porcelain Candle Pot Set',
        imageUrl: 'https://i.imgur.com/QKDCYbs.jpg',
        price: 60
      },
      {
        id: 10,
        name: 'Bayberry Pillar Candle Set',
        imageUrl: 'https://i.imgur.com/nBBizUK.jpg',
        price: 40
      }
    ]
  },
  {
    id: 3,
    title: 'Teas',
    routeName: 'teas',
    items: [
      {
        id: 1,
        name: 'Pumpkin Chai Spice Tea',
        imageUrl: 'https://i.imgur.com/OfvBuBX.jpg',
        price: 25
      },
      {
        id: 2,
        name: 'Oolong Tea Blend',
        imageUrl: 'https://i.imgur.com/8nLRrcV.jpg',
        price: 29
      },
      {
        id: 3,
        name: 'Ginger Spice Tea',
        imageUrl: 'https://i.imgur.com/q3YwaL6.jpg',
        price: 19
      },
      {
        id: 4,
        name: 'Raspberry Hibiscus White Tea',
        imageUrl: 'https://i.imgur.com/VAc96ns.jpg',
        price: 18
      },
      {
        id: 5,
        name: 'Orange Spice Flower Tea Balls',
        imageUrl: 'https://i.imgur.com/jCJaV62.jpg',
        price: 35
      },
      {
        id: 6,
        name: 'Black Tea Dried Fruit Blend',
        imageUrl: 'https://i.imgur.com/X3bb5oS.jpg',
        price: 25
      },
      {
        id: 7,
        name: 'Floral Fruits Berry Tea Blend',
        imageUrl: 'https://i.imgur.com/vh00jkB.jpg',
        price: 35
      },
      {
        id: 8,
        name: 'Twelve Flavors Wood Tea Gift Box',
        imageUrl: 'https://i.imgur.com/9uskvjb.jpg',
        price: 85
      },
      {
        id: 9,
        name: 'Stainless Steel Tea Brew Kettle & Sunflower Tea',
        imageUrl: 'https://i.imgur.com/gpwCAMp.jpg',
        price: 75
      },
      {
        id: 10,
        name: 'Mint Dried Herb Green Tea',
        imageUrl: 'https://i.imgur.com/zs6e3XN.jpg',
        price: 18
      }
    ]
  },
  {
    id: 4,
    title: 'Crystals',
    routeName: 'crystals',
    items: [
      {
        id: 1,
        name: 'Citrine Crystal Geode Slice',
        imageUrl: 'https://i.imgur.com/5xXeROz.jpg',
        price: 25
      },
      {
        id: 2,
        name: 'Amethyst Collector\'s Large Crystal Geode',
        imageUrl: 'https://i.imgur.com/l0lIHM0.jpg',
        price: 200
      },
      {
        id: 3,
        name: 'Crystal Photo Sphere',
        imageUrl: 'https://i.imgur.com/bVM2mYb.jpg',
        price: 45
      },
      {
        id: 4,
        name: 'Meditation White Quartz Crystal',
        imageUrl: 'https://i.imgur.com/BDfSmB9.jpg',
        price: 95
      },
      {
        id: 5,
        name: 'Large Sand Topaz Crystal',
        imageUrl: 'https://i.imgur.com/n9RF2AO.jpg',
        price: 135
      },
      {
        id: 6,
        name: 'Aquamarine Rough Crystal',
        imageUrl: 'https://i.imgur.com/Hd8foMf.jpg',
        price: 80
      },
      {
        id: 7,
        name: 'Amethyst Crystal Burst with Stand',
        imageUrl: 'https://i.imgur.com/8m10TMT.jpg',
        price: 99
      },
      {
        id: 8,
        name: 'Himalayan Salt Crystal Set',
        imageUrl: 'https://i.imgur.com/kSQFlNs.jpg',
        price: 60
      },
      {
        id: 9,
        name: 'Collector\'s Quartz Crystal Formation',
        imageUrl: 'https://i.imgur.com/9qKLUai.jpg',
        price: 185
      },
      {
        id: 10,
        name: 'Mixed Tumbled Crystal Meditation Stones',
        imageUrl: 'https://i.imgur.com/wWeq4w6.jpg',
        price: 25
      }
    ]
  },
  {
    id: 5,
    title: 'Sage Kits',
    routeName: 'sagekits',
    items: [
      {
        id: 1,
        name: 'Amethyst Cleansing Sage Kit',
        imageUrl: 'https://i.imgur.com/SXZK2uz.png',
        price: 32
      },
      {
        id: 2,
        name: 'Mixed Cleansing Woods Sage Kit',
        imageUrl: 'https://i.imgur.com/c8dq3Tn.jpg',
        price: 20
      },
      {
        id: 3,
        name: 'Asian Berry & Herb Sage Kit',
        imageUrl: 'https://i.imgur.com/X0IEp9E.jpg',
        price: 25
      },
      {
        id: 4,
        name: 'Hickory & Pine Sage Kit',
        imageUrl: 'https://i.imgur.com/1VRjdmv.jpg',
        price: 28
      },
      {
        id: 5,
        name: 'Sage Smudge Sticks',
        imageUrl: 'https://i.imgur.com/AjCpwgC.jpg',
        price: 40
      },
      {
        id: 6,
        name: 'Floral Essence Oils Set',
        imageUrl: 'https://i.imgur.com/EyVKzpN.jpg',
        price: 95
      },
      {
        id: 7,
        name: 'Cleansing Sage & Tarot Set',
        imageUrl: 'https://i.imgur.com/rBJoPYj.jpg',
        price: 25
      },
      {
        id: 8,
        name: 'Brass Purity Incense Burning Kettle',
        imageUrl: 'https://i.imgur.com/pcxUad2.jpg',
        price: 85
      },
      {
        id: 9,
        name: 'Stone & Healing Herbs Mortar + Pestle Kit',
        imageUrl: 'https://i.imgur.com/yKXjJ5P.jpg',
        price: 35
      },
      {
        id: 10,
        name: 'Dried Sage and Cleansing Herb Bundle',
        imageUrl: 'https://i.imgur.com/CNkQpAN.jpg',
        price: 55
      }
    ]
  }
]

export default SHOP_DATA;
